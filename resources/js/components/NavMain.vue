...
import { CELL_PADDING, NEON_BORDER_COLOR, SIDEBAR_SUBMENU_STYLE } from '@/pages/dashboard/env/sidebarConfig';
...
const getSubmenuHeight = (item: NavItem) => {
    ...
    let extraPadding = 0;
    if (SIDEBAR_SUBMENU_STYLE === 'glass') extraPadding = 32;
    if (SIDEBAR_SUBMENU_STYLE === 'neon') extraPadding = 32;
...
.neon-menu {
    ...
    border-left: 2px solid v-bind(NEON_BORDER_COLOR);
    display: flex;
...
:root:not(.dark) .neon-menu {
    ...
    border-left: 2px solid v-bind(NEON_BORDER_COLOR) !important;
...
[dir='rtl'] .neon-menu {
    border-left: none;
    border-right: 2px solid v-bind(NEON_BORDER_COLOR);
}
